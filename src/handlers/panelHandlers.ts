...
import {
  ...
  ModalSubmitInteraction, MessageFlags, AttachmentBuilder,
} from 'discord.js';
...
export async function handleFocusButton(i: ButtonInteraction) {
  ...
  if (kind === 'custom') {
    ...
    const minutes = new TextInputBuilder().setCustomId('focus:minutes').setLabel('Durée (ex. 25)').setRequired(true).setStyle(TextInputStyle.Short);
  ...
  if (hasRunning(i.user.id)) {
    return i.reply({ content: '⏳ Tu as déjà une session /focus en cours. Valide ou interrompt-la d’abord.', flags: MessageFlags.Ephemeral });
  }

  await i.deferUpdate().catch(() => {});      // ack le clic
  await startFocusSession(i as any, minutes); // publie la carte PUBLIQUE
}
